 ...
 */
export function parseNpmPackageName(raw: string): string | undefined {
  const name = raw.trim()
  if (name.length === 0) return undefined
  if (name.length > 214) return undefined
  if (/[\\\s]/.test(name)) return undefined
  if (/^(?:file|link|github|git\+|workspace|npm|http|https):/i.test(name)) return undefined
  if (name.startsWith('.') || name.startsWith('_')) return undefined
 ...
 */
export function resolvePnpmCli(from: string = import.meta.url): string {
