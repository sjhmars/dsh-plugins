 ...
 */
export function sniffImageMime(file: PendingFile): ImageMediaType | undefined {
  const fromBytes = mimeFromMagic(file.bytes)
  if (fromBytes !== undefined) return fromBytes
  if (IMAGE_TYPES.has(file.mimeType)) return file.mimeType as ImageMediaType
 ...
 */
export function splitPendingFiles(files: readonly PendingFile[]): {
  encoded: EncodedImageAttachment[]
  extras: PendingFile[]
} {
  const encoded: EncodedImageAttachment[] = []
  const extras: PendingFile[] = []
...
function mimeFromMagic(bytes: Uint8Array): ImageMediaType | undefined {
  if (bytes.length >= 8
