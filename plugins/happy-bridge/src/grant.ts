...
const ORDER: readonly RemoteGrant[] = ['watch', 'chat', 'approve', 'full']
 ...
 */
export function grantAtLeast(actual: RemoteGrant, needed: RemoteGrant): boolean {
...
export const CLAUDE_PERMISSION_MODES: ReadonlySet<string> = new Set([
  'default',
  'acceptEdits',
  'bypassPermissions',
  'dontAsk',
 ...
 */
export function classifyPermissionMode(
  mode: string,
  dshPresets: readonly string[],
): { kind: 'apply'; preset: string } | { kind: 'ignore' } | { kind: 'unknown' } {
  if (dshPresets.includes(mode)) return { kind: 'apply', preset: mode }
 ...
 */
export function splitModelCode(code: string): { provider: string; model: string } {
  const slash = code.indexOf('/')
  if (slash > 0) return { provider: code.slice(0, slash), model: code.slice(slash + 1) }
  const colon = code.indexOf(':')
  if (colon > 0) return { provider: code.slice(0, colon), model: code.slice(colon + 1) }
 ...
 */
export function messageModelCode(meta: Record<string, unknown>): string | undefined {
 ...
 */
export function messageEffort(meta: Record<string, unknown>): string | null | undefined {
  if ('effort' in meta) {
 ...
 */
export function sameHappyRuntime(previous: Config, next: Config): boolean {
 ...
 */
export function sameModelOverride(
  previous: ModelOverrideFields | undefined,
  next: ModelOverrideFields,
): boolean {
 ...
 */
export function catalogModelPick(
  meta: Record<string, unknown>,
): CatalogModelPick | undefined {
 ...
 */
export function sameCatalogPick(
  previous: CatalogModelPick | undefined,
  next: CatalogModelPick,
): boolean {
...
function catalogModelCode(meta: Record<string, unknown>): string | undefined {
...
function catalogEffort(meta: Record<string, unknown>): string | null | undefined {
  if (meta.effortLevel === null) return null
  if (typeof meta.effortLevel === 'string') return meta.effortLevel
  ...
  if (reasoning !== null && typeof reasoning === 'object' && !Array.isArray(reasoning)) {
    const current = (reasoning as { current?: unknown }).current
