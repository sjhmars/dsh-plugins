...
export async function buildSessionMetadata(
  ...
  const publishedModels = selected === undefined || models.some(model =>
    model.providerId === selected.row.providerId && model.id === selected.row.id)
    ? models
    : [selected.row, ...models]
 ...
 */
export function resolvePublishedSelection(
  models: HappyModelRow[],
  selection: HappySelection | undefined,
...
function syntheticHappyRow(selection: HappySelection): HappyModelRow {
