...
import { createUserMessage, ReasoningEffortId, type ContentBlock, type ReasoningEffortId as EffortId } from '@deepseek-ai/dsh-llm'
...
import { agentStateSnapshot, rememberCompleted } from './agent-state.ts'
...
import { catalogModelPick, classifyPermissionMode, grantAtLeast, isPublishedModelEcho, messageEffort, messageModelCode, sameCatalogPick, sameHappyRuntime, sameModelOverride, splitModelCode, type CatalogModelPick } from './grant.ts'
import {
...
import {
  answersFromCommunication, answersFromHappy, classifyInboundText,
  communicationAnswersFromWeb, mergeCustomAnswers, planReviewDeclineLabel,
} from './inbound.ts'
...
import type {
  CommunicationRpc, Config, Credentials, HappyCommunicationEntry, HappyCompletedCommunication,
  HappyCompletedRequest, HappyFormQuestion, HappyRequestEntry, PairingStatus, PendingFile, PermissionRpc,
...
interface Link {
  ...
  parked: boolean
  /** Completed permission requests, replayed in every agentState snapshot. */
  completedRequests: Map<string, HappyCompletedRequest>
  /** Completed communications forms, so the App keeps echoing the user's choice. */
  completedCommunications: Map<string, HappyCompletedCommunication>
}
...
type PendingHuman =
  | {
    ...
    id: string
    toolName: string
    /** Exact agentState entry published under {@link PendingHuman.id}. */
    request: HappyRequestEntry
  ...
  | {
    ...
    id: string
    request: HappyRequestEntry
    /** Present on the communications channel; answers arrive as the `communication` RPC. */
    communication?: HappyCommunicationEntry
    /** Per-question submitted options while waiting for the composer text after 「✏️ 自定义…」. */
...
export class HappyBridge {
  ...
  private readonly hostPin = new WeakMap<Agent, Promise<void>>()
  /** Agents that already prepend the pin onto `system-prompt/assemble`. */
  private readonly assemblePinBound = new WeakSet<Agent>()
  ...
  private async applySpawnMeta(agent: Agent, options: SpawnSessionOptions): Promise<void> {
    if (options.modelMode !== undefined) await this.applyModel(agent, options.modelMode, options.effortLevel)
    else if (options.effortLevel !== undefined) await this.applyEffort(agent, options.effortLevel)
  ...
  private queuePhoneCatalog(dshId: string, meta: Record<string, unknown>): void {
    const previous = this.inboundTail.get(dshId) ?? Promise.resolve()
    const next = previous.then(
      () => this.applyPhoneCatalog(dshId, meta),
      () => this.applyPhoneCatalog(dshId, meta),
    )
    this.inboundTail.set(dshId, next)
  }
    ...
    if (pending?.kind === 'ask') {
      ...
      if (pending.communication !== undefined) {
        this.completeCommunication(
          ...
          'answered',
          Object.fromEntries(answers.map(row => [row.id, {
  ...
  private async applyMessageMeta(link: Link, agent: Agent, meta: Record<string, unknown>, isCommand: boolean): Promise<void> {
    ...
    if (model !== undefined && !modelIsEcho) {
      ...
      if (!grantAtLeast(this.config.remoteGrant, 'full')) {
        ...
        if (current !== undefined) await this.rememberModel(agent, { provider: current.provider, model: current.model })
      } else {
  ...
  private async applyPhoneCatalog(dshId: string, meta: Record<string, unknown>): Promise<void> {
  ...
  private async applyModel(agent: Agent, code: string, effort?: string | null): Promise<void> {
  ...
  private async applyEffort(agent: Agent, effort: string): Promise<void> {
    const current = this.currentModel(agent)
   ...
   */
  private async rememberModel(agent: Agent, next: ModelOverride): Promise<void> {
   ...
   */
  private watchFirstAssemble(agent: Agent): Promise<void> {
  ...
  private pinHostPicker(agent: Agent): Promise<void> {
  ...
  private async followup(link: Link, agent: Agent, text: string, files: PendingFile[]): Promise<void> {
    ...
    if (agent.status === 'running' && link.pendingHuman === undefined) agent.steer(message)
  ...
  private onAsk(
    ...
    const phone = new Promise<AskUserQuestionAnswer>((resolve, reject) => {
      ...
      if (this.config.questionChannel === 'communications') {
        ...
        const form: HappyFormQuestion[] = request.questions.map((question, index) => ({
          id: question.id,
        ...
        const communication: HappyCommunicationEntry = {
          kind: 'form',
          createdAt: Date.now(),
          toolUseId: id,
          title: '需要你回答',
        ...
        if (pending !== undefined) {
          ...
          if (pending.kind === 'ask' && pending.communication !== undefined) {
            this.completeCommunication(
              ...
              pending.communication,
              'answered',
              communicationAnswersFromWeb(answer.answers),
            )
          } else {
   ...
   */
  private onCommunication(dshId: string, rpc: CommunicationRpc): void {
    const link = this.links.get(dshId)
    const pending = link?.pendingHuman
   ...
   */
  private pushAgentState(link: Link): void {
    const pending = link.pendingHuman
  ...
  private clearRequest(
    link: Link,
    id: string,
    status: HappyCompletedRequest['status'],
    request: HappyRequestEntry,
    reason?: string,
  ): void {
