 ...
 */
export function virtualWorkspaces(
 ...
 */
export function matchVirtualWorkspace(
  realCwd: string,
  workspaces: readonly VirtualWorkspace[],
): VirtualWorkspace | undefined {
  const want = normalizePath(realCwd)
  let best: VirtualWorkspace | undefined
  let bestLen = -1
 ...
 */
export function resolveSpawnDirectory(
  directory: string,
  workspaces: readonly VirtualWorkspace[],
): string | undefined {
...
export function listVirtualDirectory(
  requestPath: string,
  workspaces: readonly VirtualWorkspace[],
): { success: true; entries: { name: string; type: 'directory' }[] } | { success: false; error: string } {
...
export function realBasename(realPath: string): string {
