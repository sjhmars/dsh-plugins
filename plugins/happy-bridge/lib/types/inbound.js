 ...
 */
export function parseHappyInbound(plain) {
...
function userSessionEvent(record) {
    ...
    if (content.role === 'user')
        return asUnknownRecord(content.ev);
    const data = asUnknownRecord(content.data);
    if (content.type === 'session' && data.role === 'user')
        return asUnknownRecord(data.ev);
    return undefined;
}
 ...
 */
export function classifyInboundText(line, commandNames) {
 ...
 */
export function answersFromHappy(answers, questions) {
 ...
 */
export function customAnswersFromText(questions, text) {
 ...
 */
export function parsePermissionRpc(params) {
    ...
    if (!isUnknownRecord(record.updatedInput))
        return result;
