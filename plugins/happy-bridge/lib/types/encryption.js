 ...
 */
export function encryptJson(ctx, data) {
    if (ctx.variant === 'legacy')
        return encryptLegacy(data, ctx.key);
 ...
 */
export function decryptJson(ctx, data) {
    if (ctx.variant === 'legacy')
        return decryptLegacy(data, ctx.key);
 ...
 */
export function encryptB64(ctx, data) {
 ...
 */
export function decryptB64(ctx, value) {
 ...
 */
export function decryptWithEphemeralKey(encryptedBundle, recipientSecretKey) {
 ...
 */
export function wrapDataEncryptionKey(dataKey, recipientPublicKey) {
 ...
 */
export function sessionCrypto(credentials) {
 ...
 */
export function machineCrypto(credentials) {
    if (credentials.encryption.type === 'legacy') {
        return {
            ctx: { key: credentials.encryption.secret, variant: 'legacy' },
            dataEncryptionKey: undefined,
 ...
 */
export function decryptBlob(bundle, key) {
    if (bundle.length < nacl.secretbox.nonceLength + 16)
        return null;
 ...
 */
export async function deriveBlobKey(ctx) {
