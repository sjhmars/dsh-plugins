...
- **批准工具（Windows 上的 Web 和 Desktop）**：从屏幕右边滑出一张和 Windows 系统通知相近的卡片，配色跟任务栏 / 通知中心（不是应用窗口的浅色主题）。卡片上有 **拒绝**、**允许一次**，右上角有关闭 ×。
  - 点 **允许一次** 或 **拒绝**：这次直接按你的选择执行，黄框不再出现。
  - 点 **×**：马上把决定权交给网页或应用里的黄框。关掉不是拒绝。
  - 一直不点：大约 10 秒后卡片消失，同样出黄框。
- **向你提问**：只弹系统通知提醒你去看屏幕，没有这两个按钮。
- **任务结束（Desktop）**：走应用自己的通知，点一下会唤回窗口。批准不走这条，仍用上面的卡片。
...
- **提供方** `HostDesktopNotifier`：任务结束 / 提问时 Desktop 优先 Electron Notification（点击可唤回窗口）；Windows 上批准工具一律走带按钮的卡片；macOS 走 `osascript`（本仓库不验收）。
- **Windows 的两条样子（Web 与 Desktop 相同）**
  - 任务结束、提问：系统 WinRT 通知（Desktop 上任务结束优先 Electron）。
  - 批准工具：独立弹出右下角卡片（从右边滑入 / 滑出），版式对齐系统通知；点按钮把「允许一次 / 拒绝」写回 Host，点 × 或超时则交给黄框。
- **消费者**
...
`skipWhenFocused` 仅 Desktop/Electron 能判断窗口是否在前台；Web 浏览器无法从 Host 得知，始终会弹。`approvalWaitMs` 是审批卡片最长等待（默认 10000 毫秒）；关掉会立刻出黄框，超时同样出黄框，都不是拒绝。
...
- **关掉通知 ≠ 拒绝** — 点「允许一次 / 拒绝」才直接裁决。关掉卡片或等到超时，把决定权交给黄框。
- **审批卡片和黄框不能同时点** — 点了卡片按钮就不出这次黄框；黄框出现时卡片这条路已经结束。两边同时可点需要改 Harness。
- **macOS / Linux 审批通知不带按钮** — 批准还是在应用里点。Linux 仍不弹通知。
