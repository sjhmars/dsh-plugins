...
/** 用于画开始菜单小图标的产品 PNG。 */
export function toastIconPath(): string | undefined {
 ...
 */
export function buildToastXml(request: NotifyRequest): string {
  ...
  return (
    '<toast scenario="reminder" activationType="foreground" launch="dismiss">'
 ...
 */
export function parseToastStdout(stdout: string): ToastWaitAction {
  const lines = stdout.trim().split(/\r?\n/).filter(line => line !== '')
  const line = (lines.at(-1) ?? '').replace(/^\uFEFF/, '').trim()
  if (line === 'allowed-once' || line === 'rejected') return line
  return 'deferred'
}
