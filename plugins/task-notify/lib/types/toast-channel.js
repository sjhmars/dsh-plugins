...
import { DEFAULT_APPROVAL_WAIT_MS, TOAST_APP_ID, TOAST_DISPLAY_NAME, buildToastXml, toastIconPath, } from "./windows-toast.js";
...
function parseLine(line) {
    ...
    if (trimmed === 'ok')
        return { ok: true, channel: 'windows-toast' };
...
class ToastChannel {
    ...
    ensureStarted() {
        ...
        if (this.start === undefined) {
            this.start = this.spawnHelper().then(() => { this.start = undefined; }, (error) => {
    ...
    async sendNow(request) {
        ...
        try {
            ...
            if (!wait)
                return result;
            if (result.ok && (result.action === 'allowed-once' || result.action === 'rejected'))
                return result;
    ...
    onStdout(chunk) {
        ...
        while (true) {
            ...
            if (line.trim() === '')
                continue;
            if (line.includes('EventRegistrationToken'))
                continue;
 ...
 */
export function ensureWindowsToastChannel() {
    if (process.platform !== 'win32')
        return Promise.resolve();
    return channel.ensureStarted().catch(() => undefined);
}
/** 插件卸载时关掉常驻进程。 */
export function disposeWindowsToastChannel() {
    channel.dispose();
}
/**
 * 经常驻通道弹出 Windows 通知。
 * @param request - 标题、正文、可选审批等待。
 */
export function sendWindowsToast(request) {
    return channel.send(request);
}
//# sourceMappingURL=toast-channel.js.map